...
public class TestDaemonService implements DaemonService
{
	...
	private final String daemonPath = "com.thearobotics.testurcap/src/main/resources/com/thearobotics/impl/daemon/";

	//init is called when the daemon service i registered in the activator. If the daemon should be started automatically, the daemonContribution.start should be called here.  
	@Override
	public void init(DaemonContribution daemon)
	{
		try
		{
			//Installs the specified daemon file or the folder that contains it and additional files.
	...
	//Returns the object, and makes it possible for the programmer to call daemonContribution.start(), .stop()
